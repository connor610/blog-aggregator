...
## This is a RSS aggregator (or gator for short) that I wrote as part of a guided project on Boot.dev, a backend programming course.

### To use this, you'll need two things first: Postgres and Go.
...
### Next, install the blog aggregator CLI:

`go install blog-aggregator`


### this blog aggregator is only configured to run locally. It needs a config file on your computer to work.


If there isn't one already, create a gatorconfig.json file in blog aggregator's root directory, like so:
...
the blog aggregator can take a number of commands, but you must first `register` a username and `login` to set that name as the active user, so you can follow feeds.
